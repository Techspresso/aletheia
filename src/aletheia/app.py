...
def index():
    ...
    same_articles = get_similar_articles.get_articles_on_topic(topic, url, count=count)
    diff_articles = []
    if isAnti:
        diff_articles = get_similar_articles.get_articles_on_topic(
            anti_topic, url, count=count
        )
